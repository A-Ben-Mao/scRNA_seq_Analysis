...
| `6.1 scRNA_DEG_by_Group.R` | **差异基因分析（组别）** | 在**同一细胞类型**中比较不同实验组/条件下的差异基因 |
| `6.2 scRNA_DEG_by_Subtype.R` | **差异基因分析（亚型）** | 在**同一大类细胞类型**内比较不同细胞亚型之间的差异表达 |
| `7.DEG_GO_KEGG.R` | **功能富集分析** | 基于差异基因进行 GO / KEGG 通路富集分析 |
| `8.1 CellChat_Single.R` | **单组细胞通讯** | 使用 CellChat 进行不分组的**细胞通讯**分析 |
| `8.1 CellChat_Group.R` | **组间细胞通讯对比** | 对不同组别的**细胞通讯**结果进行分析 |
